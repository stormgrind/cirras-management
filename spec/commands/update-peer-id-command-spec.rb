...
module CirrASManagement
  ...
  describe UpdatePeerIdCommand do
    ...
    it "should not break if completely invalid PeerID is received" do
      ...
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost get jboss.messaging:service=ServerPeer ServerPeerID" ).once.and_return("xyz")
    ...
    it "should not break if invalid PeerID is received" do
      ...
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost get jboss.messaging:service=ServerPeer ServerPeerID" ).once.and_return("ServerPeerID=xyz")
      @exec_helper.should_not_receive(:execute)
      @cmd.execute
    end
    ...
    it "should not update peer id because it's same as previous" do
      ...
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost get jboss.messaging:service=ServerPeer ServerPeerID" ).once.and_return("ServerPeerID=1")
      @exec_helper.should_not_receive(:execute)
    ...
    it "should update peer id" do
      ...
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost get jboss.messaging:service=ServerPeer ServerPeerID" ).once.and_return("ServerPeerID=2")
      @exec_helper.should_not_receive(:execute)
      @cmd.execute
    end
    ...
    def inject_peer_id_1
      ...
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost invoke jboss.messaging:service=ServerPeer stop" ).once
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost set jboss.messaging:service=ServerPeer ServerPeerID 1" ).once
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost invoke jboss.messaging:service=ServerPeer start" ).once
    end
  end
end
