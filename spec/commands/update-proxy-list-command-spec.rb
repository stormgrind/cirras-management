...
module CirrASManagement
  ...
  describe UpdateProxyListCommand do
    ...
    it "should load 2 proxies" do
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost -u admin -p admin get jboss.web:service=ModCluster ProxyInfo" ).once.and_return("ProxyInfo={/10.210.30.227:80=Node: [1],Name: localhost.localdomain-10.211.94.34,Balancer: mycluster,Domain: ,Host: 10.211.94.34,Port: 8009,Type: ajp,Flushpackets: Off,Flushwait: 10000,Ping: 10000000,Smax: 1,Ttl: 60000000,Elected: 0,Read: 0,Transfered: 0,Connected: 0,Load: 93 Node: [2],Name: localhost.localdomain-10.210.59.133,Balancer: mycluster,Domain: ,Host: 10.210.59.133,Port: 8009,Type: ajp,Flushpackets: Off,Flushwait: 10000,Ping: 10000000,Smax: 1,Ttl: 60000000,Elected: 0,Read: 0,Transfered: 0,Connected: 0,Load: 97 , /10.210.85.220:80=Node: [1],Name: localhost.localdomain-10.211.94.34,Balancer: mycluster,Domain: ,Host: 10.211.94.34,Port: 8009,Type: ajp,Flushpackets: Off,Flushwait: 10000,Ping: 10000000,Smax: 1,Ttl: 60000000,Elected: 0,Read: 0,Transfered: 0,Connected: 0,Load: 93 Node: [2],Name: localhost.localdomain-10.210.59.133,Balancer: mycluster,Domain: ,Host: 10.210.59.133,Port: 8009,Type: ajp,Flushpackets: Off,Flushwait: 10000,Ping: 10000000,Smax: 1,Ttl: 60000000,Elected: 0,Read: 0,Transfered: 0,Connected: 0,Load: 97}")
      @cmd.get_current_proxies.size.should == 2
    end

    it "should load 1 proxy" do
      @exec_helper.should_receive(:execute).with( "#{@jboss_home}/bin/twiddle.sh -s localhost -u admin -p admin get jboss.web:service=ModCluster ProxyInfo" ).once.and_return("ProxyInfo={/10.210.30.227:80=Node: [1],Name: localhost.localdomain-10.211.94.34,Balancer: mycluster,Domain: ,Host: 10.211.94.34,Port: 8009,Type: }")
